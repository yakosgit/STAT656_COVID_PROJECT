...
### 1.1 Background:
 ...
 - This project explores some Covind-19 datasets to find and select factors/signals to be used as predictors to a specific Covid related metric, such as death rate, hospitalization rate, infection rate, ect. The main focus is on classification of an individual case as death or not. The final goal is to use the selected factors/signals to build multiple statistical models using differet classification methods to generate desired predictions for the death metric 
...
calibProbs = calibration(YtestRelevel ~ YhatTestProbLR$No, cuts = 5)
xyplot(calibProbs)
```
...
calibProbsGLMnet = calibration(YtestRelevel ~ probHatTestGlmnet, cuts = 5)
...
calibProbsLDA = calibration(YtestRelevel ~ YhatTestProbLDA$No, cuts = 5)
